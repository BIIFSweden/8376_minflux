...
# -----------------------------
# Build Plotly figure with synchronized side-by-side subplots
# -----------------------------
fig = make_subplots(
    rows=1, cols=2,
...
# -----------------------------
# Link axes so panning/zooming is synchronized
# -----------------------------
# xaxis2 and yaxis2 (right subplot) will match xaxis and yaxis (left subplot)
fig.update_xaxes(matches='x')  # Link all x-axes
fig.update_yaxes(matches='y')  # Link all y-axes

# Keep equal aspect ratio for both subplots
fig.update_yaxes(scaleanchor="x", scaleratio=1, row=1, col=1)
fig.update_yaxes(scaleanchor="x2", scaleratio=1, row=1, col=2)

fig.update_layout(
    title="MINFLUX localizations - Synchronized view (2 channels)",
...
# -----------------------------
# Show in a native WebView window (wider to accommodate side-by-side)
# -----------------------------
webview.create_window("MINFLUX viewer (synchronized)", html=html, width=1600, height=800)
webview.start()